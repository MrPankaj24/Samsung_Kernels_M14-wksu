...
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
	dev_info(dev, "%s+ power.disable_depth=%d\n",
...
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
	cs40l26->pdata.asp_scale_pct = sec_vib_inputff_get_ach_percent(&cs40l26->sec_vib_ddata);
	dev_info(cs40l26->dev, "%s calls stop playback. asp_scale_pct : %d\n",
		__func__, cs40l26->pdata.asp_scale_pct);
#endif
	if (cs40l26->pdata.asp_scale_pct < CS40L26_GAIN_FULL_SCALE)
		queue_work(cs40l26->vibe_workqueue, &cs40l26->set_gain_work);
...
#else
	dev_dbg(cs40l26->dev, "%s: effects_in_flight = %d\n", __func__,
			cs40l26->effects_in_flight);
#endif
...
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
			dev_info(dev, "GPIO%u %s edge detected\n",
					(irq1 / 2) + 1,
...
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
			dev_info(dev, "GPIO%u falling edge detected\n",
					irq1 - 8);
#else
...
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
			dev_info(dev, "GPIO%u rising edge detected\n",
					irq1 - 8);
#else
...
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
static bool samsung_is_valid_vmon(struct cs40l26_private *cs40l26, u32 vmon)
{
	u32 num;

	/* num = VMON/(2^23-1)*12.3V*1000 is between 80mV to 120mV */
	if (vmon > 54561 && vmon < 81841) {
...
static int samsung_fw_load(struct input_dev *dev, unsigned int fw_id)
{
	...
	if (ret < 0) {
		dev_err(cs40l26->dev, "%s: retry(%d), fail(%d)", __func__,
			cs40l26->sec_vib_ddata.fw.retry, ret);
		cs40l26->fw_id = 0;
	...
	for (i = 0; i < 3; i++) {
		...
		if (!ret) {
			...
			if (cs40l26->sec_vib_ddata.f0_stored) {
				ret = samsung_set_f0_stored(dev, cs40l26->sec_vib_ddata.f0_stored);
				if (ret < 0)
		...
		struct cs40l26_uploaded_effect *ueffect)
{
	...
	ret = cs40l26_get_num_waves(cs40l26, &old_nwaves);
	if (ret)
	...
	if (old_nwaves-1 != nwaves) {
		...
		cs40l26_ack_write(cs40l26, CS40L26_DSP_VIRTUAL1_MBOX_1, cmd,
				CS40L26_DSP_MBOX_RESET);
...
static void samsung_input_data_init(struct cs40l26_private *cs40l26)
{
	...
	cs40l26->sec_vib_ddata.ach_percent = cs40l26->pdata.asp_scale_pct;
...
static int cs40l26_dsp_config(struct cs40l26_private *cs40l26)
{
	...
	if (ret) {
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
		dev_err(dev, "%s:Failed to update_reg_defaults_via_pseq\n", __func__);
#endif
		return ret;
	}
	...
	if (ret) {
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
		dev_err(dev, "%s:Failed to brownout_prevention_ini\n", __func__);
#endif
		return ret;
	}
...
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
	if (of_property_read_bool(np, "samsung,f0-tracking"))
		cs40l26->pdata.is_f0_tracking = true;
	else
		cs40l26->pdata.is_f0_tracking = false;

	if (!of_property_read_s32(np, "samsung,f0-tracking-offset", &val))
		cs40l26->pdata.f0_offset = val * CS40L26_SAMSUNG_F0_OFFSET;
	else
		...
		struct cs40l26_platform_data *pdata)
{
	...
	ret = sec_vib_inputff_register(&cs40l26->sec_vib_ddata);
#else
...
int cs40l26_suspend(struct device *dev)
{
	...
	if (!cs40l26->pm_ready) {
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
		dev_info(dev, "Suspend call ignored\n");
#else
...
int cs40l26_resume(struct device *dev)
{
	...
	if (!cs40l26->pm_ready) {
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
		dev_info(dev, "Resume call ignored\n");
#else
